...
session_start();
     ...
     if(isset($_POST['submit'])){
        ...
        $returneddate = $_REQUEST['returneddate'];
        $isbn = $_REQUEST['isbn'];
       
        // Performing insert query execution
        
        $sql = "update borrow set returneddate='".$returneddate."' where isbn='".$isbn."'";
       
        if(mysqli_query($conn, $sql)){
            echo "<script>alert('Book return successful')</script>";
           
        } else{
            echo "<script>alert('Book return failed. please try again!')</script>";
...
<head>
  ...
  <!-- Basic Page Needs
	================================================== -->
  <meta charset="utf-8">
  <title>Book Return Modal | Mylibcenter</title>
    ...
    <div class="row">
      <div class="col-md-8">
        <ul class="list-inline custom-breadcrumb mb-2">
          ...
          <li class="list-inline-item text-white h3 font-secondary nasted">Book a book</li>
        </ul>
  ...
  <div class="container">
    <div class="row align-items-center">
      <div class="col-md-6 order-2 order-md-1">
        ...
        <form action="returnbook.php" method="post" class="row" name="login" onsubmit="return(validate());">
                       
                        <div class="col-12">
                            <label for="returneddate">Return date</label>
                            <input type="date" class="form-control mb-3" id="returneddate" name="returneddate" placeholder="Return date">
                        </div>
                        <div class="col-12">
                            <input type="text" class="form-control mb-3" id="isbn" name="isbn" placeholder="Book ISBN">
                        </div>
                        
<br />
                        <div class="col-12">
                            <button type="submit" name="submit" id="submit" value="submit" class="btn btn-primary">Return Book</button>
                        </div>
                    </form>
...
function validate(){
	if(document.login.returneddate.value==""){
		alert('return date is required');
		document.login.returneddate.focus();
		return false;
	}
